...
@RequiredArgsConstructor // 맴버 변수를 이용한 생성자 자동생성
    ...
    public boolean validToken(String token){
        try{
            Jwts.parser() // 토큰을 먼저 파싱(읽어들임)하고
                    .setSigningKey(jwtProperties.getSecretKey()) //복호화 시에는 암호화때 사용한 비밀키를 입력
                    .parseClaimsJws(token); // 검증 타겟이 되는 토큰 입력
            return true; // 아무일도 없으면 유효 토큰이므로 true 리턴
        }catch (Exception e){
            return false; // 파싱 및 복호화 비교중 문제가 발생하면 익셉션이 발생하므로, false리턴
        }
    }

    public Authentication getAuthentication(String token){ // 토큰 입력시 인증정보 읽어오기
        Claims claims = getClaims(token);  //getClaims() 는 아래 작성 예정
