...
public class OAuth2SuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    public static final Duration REFRESH_TOKEN_DURATION = Duration.ofDays(2); //이틀
    ...
    // @Configuration으로 등록된 설정 클래스 내부 메서드에 @Bean 어노테이션이 붙어 있다면
    ...
    public void onAuthenticationSuccess(HttpServletRequest request,
        ...
        // 인증 객체 내부 이메일로 유저 정보 조회
        User user = usersService.findByEmail((String) oAuth2User.getAttributes().get("email"));

        // 인증된 유저가 맞다면 리프레시 토큰을 생성해서 쿠키에 저장
        // 리프레시 토큰도 JWT토큰의 일종이므로 TokenProvider로 생성
        String refreshToken = tokenProvider.generateToken(user, REFRESH_TOKEN_DURATION);
        // 함수를 호출해서 뭔가 로직을 처리하는 부분은 대부분 리펙토링을 위해 외부 함수에 실행로직을 뺴 둔 경우입니다.
        saveRefreshToken(user.getId(), refreshToken);
        addRefreshTokenToCookie(request, response, refreshToken); // 사용자 측에 보내기 위한 로직

        // 전달된 리프레스토큰을 이용해 억세스토큰 생성 및 활용하기
    ...
    private void clearAuthenticationAttributes(HttpServletRequest request,
                                               HttpServletResponse response){
        // 상속 전 부모클래스쪽에 정이된 메서드는 request에 대해서만 소거가 처리되므로, 쿠키 제거를 위해 response 부분도 추가 처리
